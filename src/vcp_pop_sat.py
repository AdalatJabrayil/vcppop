...
def color( G, PRINT=8, TLIM=3600, **args ): #{
    """ 
    LANGUAGE: python3
    MODEL:    aymmetric partial ordering based model POP-S in [1]
...
def iscolorable( G, H, Q, PRINT, TLIM, SAT_FILE_PATH, ASYM ): #{
    """ 
        SAT encoding of POP for H-colorability
    """
    y = dict()
    var=1
    for v in G.nodes(): 
    ...
    for u,v in G.edges():
        ...
        cnf_formula.append( s )
        for i in range(1,H):    
            # y[i-1,u]-y[i,u] + y[i-1,v]-y[i,v] <= 1 
            s = "-%s %s -%s %s 0\n" % ( y[i-1,u], y[i,u], y[i-1,v], y[i,v] ) 
            cnf_formula.append( s )
        ...
        if H >= lenQ+2:
            ...
            for h in range(lenQ, H):
                ...
                for i in range(lenQ+1, len(pi)):
                    # y[h, pi[i] ] <= sum( y[h-1,u] for u in pi[:i] )         # see [1]
                    s = ("-%s "% y[h,pi[i]])   + ' '.join( "%s" % y[h-1,u]  for u in pi[:i] ) + " 0\n"
                    cnf_formula.append( s )
